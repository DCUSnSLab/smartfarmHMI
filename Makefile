...
health:
	...
	$(COMPOSE) ps virtual-edge --format '{{.State}}' | grep -q running && echo "✓ virtual-edge (seongju)" || { echo "✗ virtual-edge (seongju)"; ok=0; }; \
	$(COMPOSE) ps virtual-edge-jinju --format '{{.State}}' | grep -q running && echo "✓ virtual-edge (jinju)" || { echo "✗ virtual-edge (jinju)"; ok=0; }; \
