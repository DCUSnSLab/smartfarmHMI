...
8. [docs/03-architecture/component-mapping.md](./docs/03-architecture/component-mapping.md) — 기능별 컴포넌트(웹앱/애플리케이션 서버/미들웨어 서버) 책임
...
11. [docs/04-roadmap/phase2-scope.md](./docs/04-roadmap/phase2-scope.md) — 2차년도 로드맵과 현재 개발 범위
12. [docs/04-roadmap/dev-increments.md](./docs/04-roadmap/dev-increments.md) — 개발 증분 계획 (증분 0~8, DoD)
13. [docs/design/README.md](./docs/design/README.md) — 디자인 전달본 (화면 구조·디자인 토큰)
14. [docs/design/design-change-spec.md](./docs/design/design-change-spec.md) — 디자인 변경 명세 (요구사항 정합화에 따른 후속 작업)

> 문서 번호(`00-`, `01-1` 등)는 파일 헤더의 표기이고, 위 목록은 읽는 순서다. 통신 규격은 시스템 구성 바로 다음에 읽는 것이 이해하기 쉬워 `docs/03-architecture/communication-interface.md`(헤더상 03-2)를 7번에 두었다.
...
make health   # api·middleware /health + broker·db·redis 일괄 확인
```

개발 순서·완료 기준은 [docs/04-roadmap/dev-increments.md](./docs/04-roadmap/dev-increments.md), 서비스 경계 규칙(서비스 간 import 금지, `shared/schemas`만 공유)은 [docs/03-architecture/tech-stack.md](./docs/03-architecture/tech-stack.md) §3 참고. 브랜치는 develop 기준으로 작업하고 PR base도 develop이다 (develop→main은 릴리즈 PR).
...
├─ web/                         ← 웹앱 (Next.js 15)
├─ api/                         ← 애플리케이션 서버 (Django 5.2 + Channels)
├─ middleware/                  ← 미들웨어 서버 (FastAPI + aiomqtt)
├─ edge-sim/                    ← 임의 데이터 생성기 (2차년도 한정)
...
| OPN-19 | **물리 비상정지 상태는 엣지가 발행해 주기로 확정** (`estop_state` 메시지). FR-36은 2차년도 범위에서 진행한다 | docs/01-requirements/functional.md, docs/03-architecture/communication-interface.md |
...
| OPN-07 | 권한 분리 수준·역할 체계 (인증 방식은 확정 — 위 표 참고) | docs/01-requirements/non-functional.md, docs/03-architecture/tech-stack.md |
...
| OPN-22 | MQTT 브로커 배치(미들웨어 내장 vs 별도)와 인증·TLS 정책 (브로커 소프트웨어는 Mosquitto 확정) | docs/03-architecture/tech-stack.md, docs/03-architecture/system-architecture.md |
