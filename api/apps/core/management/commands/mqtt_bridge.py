...
그룹: farm_{farm_id} (특정 농장 화면) + fleet (전체 현황 화면).
      _system/health 와 alert 는 스코프와 무관하므로 전용 그룹으로 보낸다.
...
ALERTS_GROUP = "alerts"    # 〃 (consumers.MonitorConsumer 와 짝)
            ...
            async with aiomqtt.Client(host, port, keepalive=30, identifier="api-bridge") as client:
                ...
                async for message in client.messages:
                    ...
                    if farm_id == SYSTEM_SCOPE:
                        ...
                        continue
                    if stream == "alert":
                        # 알림도 같은 이유로 스코프에서 뺀다. 헤더 벨은 농장을 옮겨도
                        # 전 농장 알림을 보여야 하므로, 화면이 어느 그룹에 있든 받아야 한다.
                        await channel_layer.group_send(ALERTS_GROUP, event)
                        continue
