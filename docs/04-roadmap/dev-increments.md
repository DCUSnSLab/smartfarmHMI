...
| (병행) **main(운영) 클러스터 배포** | GEN-1389 | PR 대기 | ✅ **개통** — LoadBalancer 203.250.33.77:80, main 머지 후 Jenkins 배포 |
...
6. ~~**로봇 상태 모델 — 진행 단계와 오류를 분리할 것**~~ — ✅ **선반영 완료 (2026-08-10, GEN-1280).** `mission_state` 한 필드가 "어디까지 갔나"와 "뭐가 틀어졌나" 두 축을 겸해, 이동 중 실패하면 `moving` 이 `error` 에 덮여 어느 구간에서 멈췄는지가 사라졌다 — 재개 설계에 필요한 정보다. `phase`(4값) + `error`(객체)로 갈랐고 둘이 동시에 성립한다. 재개 설계는 이 위에서 시작한다 (`../review/edge-state-recovery.md`)
...
- **한 필드에 한 축** (스키마 0.3, GEN-1280) — 값 하나가 두 의미를 겸하면 하나가 다른 하나를 지운다. 두 곳을 갈랐다: `robot_status.mission_state` → **`phase` + `error`** (사건이 진행 단계를 덮던 문제), `estop_state.engaged: bool` → **`estop` 3값**("확인해 보니 풀림"과 "확인하지 못함"이 같은 `false` 이던 문제). `unknown` 은 발행 측·수신 측 모두 `engaged` 와 동일 취급하며 하위호환 매핑을 두지 않는다 — 매핑이 곧 결함이다. 근거는 `../review/edge-state-recovery.md`
...
- 2026-08-25 · main(운영) 개통 (GEN-1389) — 고정 IP `203.250.33.77` 를 dev 에서 운영으로 이관, dev 는 NodePort 30480 환원. 첫 운영 릴리스