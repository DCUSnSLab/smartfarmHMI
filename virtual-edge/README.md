...
**물리 비상정지 시뮬** — 접속할 때마다 `estop_state`(§4.7)를 retained 로 발행한다.
기본값은 `released`(장치를 읽어 확인했다)이고, `estop: unknown` 을 YAML 이나
`VEDGE_ESTOP` 환경 변수로 켜면 **"엣지가 장치를 못 읽는" 고장**을 재현한다.

```bash
VEDGE_ESTOP=unknown docker compose up   # 서버가 안전측(눌림)으로 판정하는지 확인
```

0.2 는 "확인해 보니 풀림"과 "확인하지 못함"이 같은 `false` 였다. 이 스위치는
엣지 재시작 때 안전이 조용히 열리던 결함이 되살아나는지 보는 장치다.
