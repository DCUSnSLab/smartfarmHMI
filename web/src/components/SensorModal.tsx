  ...
  return (
      ...
      footer={
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 rounded-xl bg-gray-100 py-3 text-14 font-extrabold text-gray-400"
          >
            영점 보정
          </button>
          <PlannedChip basis="FR-39 보정" />
          <button onClick={onClose} className="rounded-xl bg-primary px-5 py-3 text-14 font-extrabold text-white">
            닫기
          </button>
        </div>
      }
    >
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div>
          <div className="text-12 font-bold text-gray-500">현재값</div>
          <div className="text-24 font-extrabold">
            {sensor.value != null ? sensor.value.toFixed(1) : "—"}
            <span className="ml-0.5 text-12 font-bold text-muted">{meta.unit}</span>
          </div>
        </div>
        <div>
          <div className="text-12 font-bold text-gray-500">통신 상태</div>
          <div className="mt-1.5">
            {connState ? (
              <StatusDot sev={CONN_STYLE[connState]?.sev ?? "info"} label={CONN_STYLE[connState]?.label} />
            ) : (
              <span className="text-13 font-semibold text-muted">—</span>
            )}
          </div>
        </div>
        <div>
          <div className="text-12 font-bold text-gray-500">센서 자체 상태</div>
          <div className="mt-1 text-13.5 font-bold">
      ...
      <div className="rounded-2xl bg-surface p-4">
        <div className="mb-2 text-13 font-extrabold">최근 24시간 추이 (30분 평균)</div>
        {history === null ? (
          <div className="flex h-[180px] items-center justify-center text-13 font-semibold text-muted">
            불러오는 중…
          </div>
        ) : (
          <LineChart
