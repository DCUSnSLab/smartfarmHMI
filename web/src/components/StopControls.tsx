...
export function StopButton({ canStop }: { canStop: boolean }) {
  ...
  return (
    <>
      {/* 라벨은 창 너비 기준으로 줄인다 (글자 크기와 무관) — 줄인 라벨도 글자 크기에
          따라 함께 커진다. 전체 명칭은 aria-label 과 확인 모달이 유지한다 */}
      <button
      ...
      {confirming && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 px-6">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-xl">
            ...
            <h2 className="mb-2 text-20 font-extrabold">원격 전체 정지를 실행할까요?</h2>
            <p className="mb-6 text-14 font-semibold leading-relaxed text-gray-600">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl bg-gray-100 py-3.5 text-15 font-extrabold text-gray-600"
              >
                취소
              </button>
              <button
                ...
                className="flex-1 rounded-xl bg-status-warningDark py-3.5 text-15 font-extrabold text-white disabled:opacity-60"
              >
      ...
      {stops.physical_estop && (
        <div className="flex items-center gap-3 bg-gradient-to-r from-[#D32030] to-status-warning px-6 py-3.5 text-white">
          ...
          <div className="flex-1">
            ...
            <div className="text-12.5 font-semibold opacity-90">
        ...
        <div className="flex items-center gap-3 bg-gradient-to-r from-[#E07800] to-status-caution px-6 py-3.5 text-white">
          ...
          {canRelease && (
            <button
              ...
              className="rounded-xl bg-white px-4 py-2 text-13.5 font-extrabold text-[#E07800] disabled:opacity-60"
            >
              {busy ? "해제 중…" : "원격 정지 해제"}
            </button>
          )}
